...
void vsplit_container_render(struct Render2D *render, ContainerVSplit *container) {
  ...
  ContainerVSplit *vsplit = container->next;
  while(vsplit) {
    ASSERT(vsplit->header.type == CONTAINER_VSPLIT);
    container_render(render, vsplit->right);
    vsplit = vsplit->next;
  }
}
...
void docker_init(struct CoreWindow *window) {
  ...
  Container *split = docker.root;
  while(split) {
    ASSERT(split->header.type == CONTAINER_VSPLIT);
    print_rect(split->header.rect);
    split = (Container *)split->vsplit.next;
  }
